...
#
source /etc/profile
sudo cp /data/software/php-7.1.4/php.ini-development /usr/local/php/etc/php.ini
...
sudo cp /data/software/php-7.1.4/sapi/fpm/init.d.php-fpm /etc/init.d/php-fpm
...
memcached：
```sh
wget https://launchpad.net/libmemcached/1.0/1.0.18/+download/libmemcached-1.0.18.tar.gz -O /tmp/libmemcached-1.0.18.tar.gz
tar zxvf /tmp/libmemcached-1.0.18.tar.gz
...
cd /tmp/memcached-ext && phpize && ./configure --with-libmemcached-dir=/usr/local/libmemcached && sudo make && sudo make install
```
